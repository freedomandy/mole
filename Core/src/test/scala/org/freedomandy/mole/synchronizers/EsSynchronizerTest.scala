...
/**
  ...
  */
class EsSynchronizerTest extends Matchers with FunSpecLike with BeforeAndAfterAll {
  describe("Test for Elastic search synchronizer") {
    ignore("upsert") {
      val configString =
        """
          |{
      ...
      val esConnector = EsSynchronizer

      esConnector.upsertData(config)(df, "_1")
      ...
      esConnector.upsertData(config)(updatedDF, "_1")
