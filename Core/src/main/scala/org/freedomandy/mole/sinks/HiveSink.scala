...
object HiveSink extends Sink {
  case class HiveParams(database: String, table: String, dateCasting: Boolean, timeCasting: Boolean,
                        decimalCasting: Boolean, saveFormat: Option[String])
  object HiveParams {
  ...
  override def upsert(config: Config)(upsertDF: DataFrame, keyField: String): Unit = {
    ...
    val currentDf = session.sql(s"SELECT * FROM ${hiveParams.database}.${hiveParams.table}").select(columns.head, columns.tail:_*)
    ...
    saveToHive(ColsCast(resultDF, hiveParams.dateCasting, hiveParams.timeCasting, hiveParams.decimalCasting), hiveParams.database, tempTableName, hiveParams.saveFormat)

    // Synchronize records from temp table to original table
    session.sqlContext.table(s"${hiveParams.database}.$tempTableName").write.format(hiveParams.saveFormat.getOrElse("parquet")).mode("overwrite").insertInto(s"${hiveParams.database}.${hiveParams.table}")
  ...
  override def overwrite(config: Config)(dataFrame: DataFrame, keyField: String): Unit = {
    ...
    saveToHive(ColsCast(dataFrame, hiveParams.dateCasting, hiveParams.timeCasting, hiveParams.decimalCasting), hiveParams.database, hiveParams.table, hiveParams.saveFormat)
  }

  def append(config: Config)(dataFrame: DataFrame, keyField: String = "IGNORED"): Unit = {
    val hiveParams = HiveParams(config)

    ColsCast(dataFrame, hiveParams.dateCasting, hiveParams.timeCasting, hiveParams.decimalCasting).write.mode("append").saveAsTable(s"${hiveParams.database}.${hiveParams.table}")
  }

  protected def ColsCast(dataFrame: DataFrame, dateToString: Boolean = false,
                         timestampToLong: Boolean = false, decimalToDouble: Boolean = false): DataFrame = {
