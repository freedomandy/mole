...
case class Monitor(module: Module) {
  ...
  def executeWithMonitor(df: DataFrame, monitorConfig: Config, exceptionConfigs: List[Config], action: DataFrame => DataFrame): Option[DataFrame] = {
    ...
    try {
      supervisor.supervise(df, monitorConfig).map[DataFrame](action)
  ...
  def handle(monitorConfig: Config): Option[DataFrame] = {
    ...
    for {
      ...
      transformedDF <- executeWithMonitor(sourceDf, monitorConfig.getConfig("Transform.preStart"), exceptionConfigs, module.transformer.run)
      result <- executeWithMonitor(transformedDF, monitorConfig.getConfig("Load.preStart"), exceptionConfigs, df => { module.loader.run(df); df })
    } yield result
  }
}
