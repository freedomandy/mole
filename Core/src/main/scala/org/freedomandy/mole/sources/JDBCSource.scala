...
object JDBCSource extends Source {
  ...
  override def get(session: SparkSession, config: Config): Option[DataFrame] = {
    ...
    val (driverString, connectionString) =
      dbType match {
        ...
        case "TeraData" =>
          ...
          val connection = s"jdbc:teradata://$uri/database=$database, TMODE=ANSI, CHARSET=UTF8, COLUMN_NAME=ON, MAYBENULL=ON, user=$user, password=$password"
    ...
    try {
      ...
      Some(session.read.format("jdbc").option("driver", driverString).option("url", connectionString).option("dbtable", query).load())
    } catch {
      case e: Exception =>
