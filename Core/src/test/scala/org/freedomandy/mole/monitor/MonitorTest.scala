...
class MonitorTest extends Matchers with FunSpecLike with BeforeAndAfterAll {
  ...
  describe("Integration Test") {
    ...
    it ("exception handling") {
      ...
      val baseExceptionMonitorResult = Monitor(Module.apply(session, jobConfig)).executeWithMonitor(df,
      ...
      val UnSupExceptionMonitorResult = Monitor(Module.apply(session, jobConfig)).executeWithMonitor(df,
