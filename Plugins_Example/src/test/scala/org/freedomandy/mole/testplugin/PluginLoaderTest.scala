  ...
  ignore("Test for sink plugin") {
    def getNumberOfStoredData(session: SparkSession): Long = {
      ...
      val readConfig = ReadConfig(
        Map("uri" -> "mongodb://127.0.0.1:27017/test.test_result", "readPreference.name" -> "secondaryPreferred")
      )
      val mongoIdSet =
        MongoSpark.load(session.sparkContext, readConfig).flatMap(document => Set(document.getString("_1")))

      mongoIdSet.count()
    }
    def cleanMockData(): Unit = {
      val writeConfig =
        WriteConfig(Map("uri" -> "mongodb://127.0.0.1:27017/test.test_result", "writeConcern.w" -> "majority"))
