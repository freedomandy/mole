...
object Filler extends FlowStage {
  def fillLastValue(lastValue: Double, index: Int, fieldIndex: Int, rddList: List[(Row, Int)], numOfSteps: Long): List[(Row, Int)] = {
    if (index <= numOfSteps) {
  ...
  def fillForward(session: SparkSession, df: DataFrame, sessionKeyFields: Set[String], fillField: Int, sortField: Int): DataFrame = {
    ...
    val sortedGroup = pairRDD.aggregateByKey(List[Row]())(
